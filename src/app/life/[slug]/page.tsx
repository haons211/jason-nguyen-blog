...
import { getAllLifePostSlugs, getLifePostBySlug, getAllLifePosts } from "@/lib/life";
...
export async function generateStaticParams() {
  const posts = await getAllLifePostSlugs();
  return posts;
}

// Generate metadata for each life post
...
export default async function LifePost(props: SlugPageProps) {
  // Await the params
  const params = await props.params;
  const slug = params.slug;
  
  const { meta, content } = await getLifePostBySlug(slug);
  const allPosts = await getAllLifePosts();
          ...
          <div className="min-w-0">
            <article className="prose lg:prose-xl mx-auto max-w-none">
              ...
              <header className="mb-10 not-prose">
                <Link 
                  href="/life" 
                  className="text-sm text-purple-600 flex items-center mb-6 hover:underline"
                >
                ...
                {meta.tags && meta.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-8">
                    {meta.tags.map((tag) => (
                      <span
                        key={tag}
                        className="bg-purple-50 text-purple-700 text-sm px-3 py-1 rounded-full border border-purple-100"
                      >
                        #{tag}
                      </span>
                    ))}
                ...
                <div className="h-1 w-20 bg-purple-600 mt-8"></div>
              </header>
